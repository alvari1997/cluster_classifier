...
class Depth_Cluster{
    ...
    std::array<int, packet_size>  Packet_cluster(py::array_t<double> input_array, py::array_t<int> pred_array){
    	// use np.reshape(-1) to strip the 2D image to 1d array
        ...
        for (size_t idx = 0; idx < this->width; idx++){
            //for (size_t idx = 0; idx < this->width; idx++){
            for (size_t idy = (int)this->height/2-2; idy < this->height; idy++){
